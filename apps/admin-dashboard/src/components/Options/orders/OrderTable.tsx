...
export const OrderTable = () => {
  ...
  const ExpandedComponent: React.FC<
    ExpanderComponentProps<OrderDetailsType>
  > = ({ data }) => {
    return (
      <>
  ...
  useEffect(() => {
    ...
    const q = query(
      ...
      orderBy('timeStamp'),
      startAt((currentPageNumber - 1) * pageSize),
      limit(pageSize)
    );
    ...
    return () => unsub();
  }, [currentPageNumber, dispatch, pageSize]);
...
const columns = [
  ...
  },
  {
    ...
    selector: (data: OrderDetailsType) =>
      data.items[0].size + '/' + data.items[0].count,
  },
  {
