...
const ProductPage = () => {
  ...
  const AddToCard = () => {
    if (!product) return;
    const productid = cartProductList.filter(
    ...
    if (productid.length > 0 && productSize.length > 0) {
      ...
      if (size) {
        const target = product.sideImages.find(
          (img) => img.sideName === 'Front'
        );
        if (!target) return;
        const id = uuidv4();
        const localCart = {
  ...
  const getSellersProducts = async () => {
    ...
    dispatch(
      setSellerProductsList(docSnap.docs.map((doc) => ({ ...doc.data() })))
    );
  };
  ...
  return (
    <div>
      ...
      <div
        className={
          medium ? 'grid grid-cols-2 mx-10 space-x-10' : 'mx-10 grid gap-5'
        }
      >
        <ProductDetails />
        <Ratings />
      </div>
      <Typography className="py-10 text-center" variant="h5">
        Recommended Products
      </Typography>
      {productsList && (
        <div className={medium ? 'grid grid-cols-4 mx-20 gap-8' : 'grid gap-8'}>
          {productsList.map((item) => {
            return (
              <ProductsCard
                key={item.productId}
                comparedPrice={item.comparePrice}
                images={item.sideImages}
                price={item.price}
                productName={item.productName}
                sizeArray={item.sizeAvailable}
              />
            );
          })}
        </div>
